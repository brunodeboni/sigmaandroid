$(function( $ ){
	$.fn.accessStories = function(settings){
		...
		var defaults = {
			...
			speed: "normal",
			// slideshow interval
			slideShowInterval: 10000000000000,
			// delay before slide show begins
			slideShowDelay: 1000000000000,
			// theme
			theme: "default",
		...
		return this.each(function(){
			...
			var container = {
				...
				_headline: $("<div class='jqans-headline'></div>").html(["<p><strong>", settings.title, "</strong> ", settings.subtitle, "</p>"].join("")),
				_content: $("<div class='jqans-content'></div>"),
				...
				selector: function(width){
					...
					var o = $("<div class=\"jqans-stories-selector\"></div>");
					o.append("<ul>"+ s +"</ul>");
					_storyIndictor = $(o.find("ul"));
				...
				set: function(position){
					...
					var storyIndictor = $(_storyIndictors[position]);
					var _content = $("div.jqans-content", container);
					var img = $('<ul class="bxslider"></ul>');
					var lis = $('.bxslider li');
					var para = $('<div></div>');
					var title = $(settings.contentTitle + " a", story).attr('title') || $(settings.contentTitle, story).text();
					//img.attr('src', $('img', story).attr('longdesc') || $('img', story).attr('src'));
					img.append(lis);
					para.html("<h1>" + title + "</h1>" + "<p>" + $(settings.contentDescription, story).html() + "</p>");
					_content.empty();
					_content.append(img);
			...
			var pagination = {
				...
				init: function(){
					if (stories.length > settings.slideBy) {
						...
						this._storyWidth = $(stories[0]).outerWidth(true);
				...
				draw: function(){
					...
					var _viewAll = $("<div class=\"jqans-pagination\"></div>").html(["<div class=\"jqans-pagination-count\"><span class=\"jqans-pagination-count-start\">1</span> - <span class=\"jqans-pagination-count-end\">", settings.slideBy, "</span> of <span class=\"jqans-pagination-count-total\">", stories.length, "</span> total</div><div class=\"jqans-pagination-controls\"><span class=\"jqans-pagination-controls-back\"><a href=\"#\" title=\"Back\">&lt;&lt; Back</a></span><span class=\"jqans-pagination-controls-next\"><a href=\"#\" title=\"Next\">Next &gt;&gt;</a></span></div>"].join(""));
					_this.after(_viewAll);
					
					var _next = $(".jqans-pagination-controls-next > a", _viewAll);
					var _back = $(".jqans-pagination-controls-back > a", _viewAll);
				...
				to: function(page){
					...
					var startAt = $(".jqans-pagination-count-start", viewAll);
					var endAt = $(".jqans-pagination-count-end", viewAll);
			...
			var slideshow = {
				...
				attach: function(){
					...
					var that = $(_this).parent("div.jqans-wrapper");
					that.hover(function(){
